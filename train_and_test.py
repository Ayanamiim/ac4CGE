...
from models.Randomforest_Model import RandomForest_Model
import torch
...
def train_and_test(pos_train_fa, neg_train_fa, test_fa, dataset, out_path, save_path=None):
    ...
    print("get_feature_wrapper")#delete
...
def select_features(data_wrapper, dataset, reference_number=20):
    ...
    from parameters import dataset_parameters_dict
    model_func_args = dataset_parameters_dict[dataset]["randomforest"]
    model = RandomForest_Model()
    all_feature_list = ["BPB", "NPPS-xi3", "NPPS-xi2", "NPPS-xi1", "NPS", "CTD", "PseKNC", "NCP-ND", "EIIP","NA"]
...
def get_seq_features(datawrapper, seq_features_list) -> FeatureWrapper:
    ...
    from features.NA_feature import generate_na_code
    ...
    for use_seq_feature in seq_features_list:
        ...
        elif use_seq_feature == "NA":
            feature_wrapper.add_feature_df(datawrapper, feature_func=generate_na_code, feature_name=use_seq_feature)
        else:
            print("error!")
            return
    return feature_wrapper
